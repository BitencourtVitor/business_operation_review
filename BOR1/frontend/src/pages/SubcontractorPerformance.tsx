...
export default function SubcontractorPerformance({ telaId: _telaId, usuarioId: _usuarioId, role: _role, isResponsavelPelaTela: _isResponsavelPelaTela }: SubcontractorPerformanceProps) {
  ...
  */
  ...
  const removeLotFromProjectName = (projectName: string) => {
    const name = (projectName || '').trim();
    if (!name) return name;
    const withoutCommunity = name
      .replace(/\bemerald\s+run\b/ig, '')
      .replace(/\s*-\s*$/g, '')
      .trim();

    const cleaned = withoutCommunity
      .replace(/[\[\(]\s*(lot|lote|bld|building)\s*([^\]\)]*)[\]\)]/ig, '$2')
      .replace(/\b(lot|lote|bld|building)\b\s*/ig, '')
      .replace(/\s*-\s*/g, ' ')
      .replace(/\s{2,}/g, ' ')
      .trim();
