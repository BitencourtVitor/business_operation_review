...
func main() {
	...
	subcontractorHandler     := handler.NewSubcontractorHandler(subcontractorService)
	fuelHandler              := handler.NewFuelHandler(samsaraRepo, wexRepo)
	permitRowHandler         := handler.NewPermitRowHandler(permitRowService)
	serviceRequestHandler    := handler.NewServiceRequestHandler(serviceRequestService)
	timesheetRowHandler      := handler.NewTimesheetRowHandler(timesheetRowService)
	projMonHvacHandler       := handler.NewProjectMonitoringHvacHandler(projMonHvacService)
	employeeNameHandler      := handler.NewEmployeeNameHandler(employeeNameService)
	takeoffHandler           := handler.NewTakeoffWorkHandler(takeoffService)
	destaqueHandler          := handler.NewDestaqueHandler(destaqueService)
	oportunidadeHandler      := handler.NewOportunidadeHandler(oportunidadeService)
