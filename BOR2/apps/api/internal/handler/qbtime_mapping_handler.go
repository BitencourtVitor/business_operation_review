...
// POST /qbtime/mapping/restore?company=hvac
// One-off recovery: re-creates the auto-matched mappings (score >= 0.85) that
// were cleared, reusing the original matcher. Idempotent (ON CONFLICT DO NOTHING).
func (h *QBTimeMappingHandler) Restore(c *fiber.Ctx) error {
	company := c.Query("company")
	if company == "" {
		return fiber.NewError(fiber.StatusBadRequest, "company is required")
	}
	...
	groups := map[string]*qbtGroup{}
	rows, err := h.db.Query(ctx, `SELECT jobcode_path::text FROM qbtime_timesheets WHERE company=$1 GROUP BY jobcode_path`, company)
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, err.Error())
	}
	seen := map[string]bool{}
	...
	for _, cu := range h.loadCustomers(ctx, company) {
		...
		js := index[jk]
		if js == nil {
			js = &qboJS{rawJobsite: jobsite, lots: map[int][]Suggestion{}}
			index[jk] = js
	...
	for _, g := range groups {
		...
		for _, lt := range g.lots {
			...
			case private:
				sug = suggestPrivate(lt.lot, qboByNum)
			}
			if sug != nil && sug.Score >= 0.85 {
				if _, err := h.db.Exec(ctx, `
					INSERT INTO qbtime_project_mappings (company, address_key, customer_id, updated_at)
					VALUES ($1,$2,$3,now()) ON CONFLICT (company, address_key) DO NOTHING`,
					company, lt.addressKey, sug.CustomerID); err == nil {
					inserted++
				}
			}
		}
	}
	return c.JSON(fiber.Map{"company": company, "inserted": inserted})
}
