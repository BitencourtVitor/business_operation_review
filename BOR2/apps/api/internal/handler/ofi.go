...
// ─── GET /ofi/live ────────────────────────────────────────────────────────────
// Computes OFI scores from the current state of forecast_core without writing
// to the database. Used by the frontend to show a provisional score for the
// current month while it is still in progress.

func (h *OFIHandler) LiveScores(c *fiber.Ctx) error {
	...
	startOfMonth := fmt.Sprintf("%04d-%02d-01", year, month)
	endOfMonth   := time.Date(year, time.Month(month+1), 0, 0, 0, 0, 0, time.UTC).Format("2006-01-02")

	type obraInfo struct {
		ID           string
		Storage      bool
		QBTime       bool
		BuilderTrend bool
	...
	rows, err := h.db.Query(ctx, `
		SELECT fc.id,
		       COALESCE(fc.storage,false),
		       COALESCE(fc.qb_time,false),
		       COALESCE(fc.buildertrend,false),
		       CASE
		         WHEN NULLIF(fc.name,'')     IS NOT NULL AND NULLIF(fc.lote_bld,'') IS NOT NULL THEN fc.name     || ' - ' || fc.lote_bld
		         WHEN NULLIF(fc.name,'')     IS NOT NULL                                        THEN fc.name
		         WHEN NULLIF(fc.job_site,'') IS NOT NULL AND NULLIF(fc.lote_bld,'') IS NOT NULL THEN fc.job_site || ' - ' || fc.lote_bld
		...
		WHERE  fc.previous_start_date >= $1
		  AND  fc.previous_start_date <= $2`,
		startOfMonth, endOfMonth,
	)
	if err != nil {
		return ofiInternalErr(c, "load obras for live scoring", err)
	}
	var obras []obraInfo
	for rows.Next() {
		var o obraInfo
		if err := rows.Scan(&o.ID, &o.Storage, &o.QBTime, &o.BuilderTrend, &o.ProjectName, &o.Address); err == nil {
	...
	for _, o := range obras {
		...
		results = append(results, ofiRow{
			ID:             o.ID,
			ObraID:         o.ID,
			ReferenceMonth: month,
			ReferenceYear:  year,
			FieldwireScore: fwScore,
			MachinesScore:  mScore,
	...
	// Sort by total score descending (mirrors the List endpoint)
