...
export function useBudgetProjects(params: { company: string; status?: BudgetStatus; periodStart?: string; periodEnd?: string }) {
  return useQuery({
    queryKey: ["budget-projects", params.company, params.status ?? "all", params.periodStart ?? "", params.periodEnd ?? ""],
