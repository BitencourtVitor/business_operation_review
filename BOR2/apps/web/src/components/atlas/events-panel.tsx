...
import {
  ...
  useDeleteAtlasEvent, useUpdateAtlasEvent,
 ...
 */
function Photos({ jobsiteId, eventId }: { jobsiteId: string; eventId: string }) {
    ...
    return (
      <div className="rounded-lg border border-dashed border-border/60 p-10 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          They start on the drawing: open a sheet, drop a note on the point, and it
          shows up here.
        </p>
      </div>
    )
          ...
          <div key={e.id} className="rounded-lg border border-border/60 bg-card p-3">
            ...
            {isOpen && (
              <div className="mt-3 flex flex-col gap-3">
                ...
                <Photos jobsiteId={jobsiteId} eventId={e.id} />
                <Thread
                  event={e}
                  jobsiteId={jobsiteId}
                  canWrite={canWrite}
