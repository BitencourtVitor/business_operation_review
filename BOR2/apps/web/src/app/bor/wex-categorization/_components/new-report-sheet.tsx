...
export function NewReportSheet({ open, onClose, company, onSave }: {
  ...
  const displayRows = results.flatMap((r, i) => r.obras.map((o, j) => ({ r, o, key: `${i}-${j}` })))
      ...
      <DialogContent showCloseButton className="flex max-h-[90vh] flex-col gap-0 p-0" style={{ width: "min(92vw, 72rem)", maxWidth: "none" }}>
        ...
        <div className="flex flex-1 flex-col gap-4 overflow-y-auto p-6 no-scrollbar">
          ...
          <div className="rounded-xl border border-border bg-card/60">
            ...
            <div className="grid grid-cols-1 gap-4 p-4 sm:grid-cols-[1fr_1fr_auto]">
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-border bg-muted/40 hover:bg-muted/40">
                      {["Date","Day","Driver (WEX)","QB Time Name","Total Cost","Period","Obra","Allocated","Days Visited","City"].map(h => (
                        <TableHead key={h} className="whitespace-nowrap text-[11px] uppercase tracking-wide">{h}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {displayRows.map(({ r, o, key }) => (
                      <TableRow key={key} className={cn("border-border/50", r.isOffice && "bg-amber-500/5")}>
                        ...
                        <TableCell className="whitespace-nowrap text-xs text-muted-foreground">{fmtDate(r.periodStart)} – {fmtDate(r.periodEnd)}</TableCell>
                        <TableCell className={cn("max-w-56 truncate text-xs", r.isOffice && "italic text-amber-500")} title={o.obra}>{o.obra}</TableCell>
                        <TableCell className={`whitespace-nowrap text-right text-xs font-bold tabular-nums text-primary ${blur}`}>{!r.isOffice ? `$${o.allocatedCost.toFixed(2)}` : "—"}</TableCell>
                        <TableCell className="max-w-40 truncate text-xs text-muted-foreground" title={o.daysVisited.join(", ")}>{o.daysVisited.length ? o.daysVisited.map(d => fmtDate(d)).join(", ") : "—"}</TableCell>
