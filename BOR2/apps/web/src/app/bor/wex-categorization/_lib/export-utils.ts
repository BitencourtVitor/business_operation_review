...
import { COMPANY_LABEL, fmtDate, normalizeResultRow } from "./wex-algorithm"
import type { AnyResultRow, Company } from "./wex-algorithm"
...
// Two-level pivot: level 0 = transaction (date + total cost), level 1 = obra
// rows nested under it (one row per obra, cost = sum across every day visited
// within that transaction's coverage period, "days visited" kept as metadata —
// never a separate row per day). Nesting uses Excel outline/grouping levels
// so the sheet opens collapsible per transaction, same as a native pivot drill-down.
export function exportToExcel(rawRows: AnyResultRow[], company: Company) {
  const rows = rawRows.map(normalizeResultRow)
  const header = [
    "Transaction Date","Weekday","Card Number","Driver (WEX)","Driver (QB Time)",
    "Total Fuel Cost","Period Start","Period End","Obra","Allocated Cost","Days Visited",
  ]
  const data: (string | number)[][] = [header]
  const levels: number[] = [0]

  for (const r of rows) {
    data.push([r.txDate, r.weekday, r.cardNumber, r.driverWexName, r.driverQbName, r.totalFuelCost, fmtDate(r.periodStart), fmtDate(r.periodEnd), "", "", ""])
    levels.push(0)
    for (const o of r.obras) {
      data.push(["", "", "", "", "", "", "", "", o.obra, o.allocatedCost, o.daysVisited.map(fmtDate).join(", ")])
      levels.push(1)
    }
  }

  const ws = XLSX.utils.aoa_to_sheet(data)
  ws["!rows"] = levels.map(level => ({ level }))

  // Consolidated sheet — total fuel cost per obra/address, summed across every
  // transaction that allocated cost to it (excludes "Office", same as the PDF summary).
  const obraTotals = new Map<string, number>()
  for (const r of rows)
    for (const o of r.obras)
      if (o.obra !== "Office") obraTotals.set(o.obra, (obraTotals.get(o.obra) ?? 0) + o.allocatedCost)
  const sortedObras = [...obraTotals.entries()].sort((a, b) => b[1] - a[1])
  const grandTotal = sortedObras.reduce((s, [, cost]) => s + cost, 0)

  const summaryData: (string | number)[][] = [
    ["Obra / Address", "Total Fuel Cost"],
    ...sortedObras.map(([obra, cost]) => [obra, cost]),
    ["Total", grandTotal],
  ]
  ...
  XLSX.utils.book_append_sheet(wb, summaryWs, "By Address")
  XLSX.writeFile(wb, `wex_${company}_${new Date().toISOString().slice(0, 10)}.xlsx`)
}

export function exportToPdf(report: WexReport, rawResults: AnyResultRow[]) {
  const results = rawResults.map(normalizeResultRow)
  ...
  for (const r of results)
    for (const o of r.obras)
      if (o.obra !== "Office") obraMap.set(o.obra, (obraMap.get(o.obra) ?? 0) + o.allocatedCost)
  const obraList = [...obraMap.entries()].sort((a, b) => b[1] - a[1])

  const rowsHtml = results.flatMap(r => r.obras.map(o => `
    <tr class="${r.isOffice ? "office" : ""}">
      ...
      <td class="${r.isOffice ? "amber" : ""}">${o.obra}</td>
      <td class="num">${o.daysVisited.length || "—"}</td>
      <td class="num">${!r.isOffice ? `$${o.allocatedCost.toFixed(2)}` : "—"}</td>
...
<table>
  <thead><tr>
    ...
    <th>Total Cost</th><th>Obra</th><th>Days</th><th>Allocated</th><th>City</th>
  </tr></thead>
