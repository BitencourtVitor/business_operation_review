  ...
  return (
    <div className="flex h-7 items-center gap-1 rounded-md border border-input bg-transparent px-2 dark:bg-input/30"
      ...
      <input
        ...
        placeholder="Filter..."
        className="w-24 bg-transparent text-xs outline-none placeholder:text-muted-foreground/50"
        onKeyDown={e => { if (e.key === "Escape") { onChange(""); setOpen(false) } }}
      />
      <button onClick={close} className="shrink-0 text-muted-foreground transition-colors hover:text-foreground">
        <X className="h-3 w-3" />
      </button>
    </div>
  )
    ...
    <AccordionItem value={cat.id}>
      <div className="flex items-center gap-2 px-3 py-1.5 transition-colors hover:bg-muted/30">
        {editing ? (
          ...
          <>
            ...
            <span className="flex min-w-0 flex-1 items-center gap-1.5">
              ...
              <span className="shrink-0 text-xs font-normal text-muted-foreground">· {subs.length}</span>
            </span>
            <span className="flex shrink-0 items-center gap-1.5 text-xs text-muted-foreground">
              {isExpanded && <SearchToggle value={subSearch} onChange={setSubSearch} />}
              {isExpanded && <span className="shrink-0 text-muted-foreground/50">Maximum value:</span>}
              <span className="shrink-0 tabular-nums">{maxDisplay}</span>
            </span>
            <Tip label="Edit">
        ...
        <AccordionPrimitive.Header className="flex shrink-0">
          <AccordionPrimitive.Trigger onClick={() => setIsExpanded(e => !e)}
            className="group flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
      ...
      <AccordionContent className="px-3">
        {subs.length === 0 ? (
          ...
          const filtered = subSearch ? subs.filter(s => s.display_name.toLowerCase().includes(subSearch.toLowerCase())) : subs
...
function CategoriesManager({ company, projectType, showNew, onCloseNew }: {
  ...
  const filteredUncategorized = uncatSearch ? uncategorized.filter(s => s.display_name.toLowerCase().includes(uncatSearch.toLowerCase())) : uncategorized
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3">
        ...
        <div className={cn("flex flex-col overflow-hidden rounded-lg border border-border/60 bg-card/60", uncatOpen ? "min-h-0 flex-1" : "shrink-0")}>
          <div className="flex shrink-0 items-center gap-2 px-3 py-2 text-sm">
            <button onClick={() => setUncatOpen(o => !o)} className="flex min-w-0 flex-1 items-center gap-1.5 text-left transition-colors hover:opacity-80">
              <span className="font-semibold">Subcontractors without category</span>
              <span className="shrink-0 text-xs font-normal text-muted-foreground">· {uncategorized.length}</span>
            </button>
            {uncatOpen && <SearchToggle value={uncatSearch} onChange={setUncatSearch} />}
            <button onClick={() => setUncatOpen(o => !o)} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
              <ChevronDown className={cn("h-4 w-4 transition-transform", uncatOpen && "rotate-180")} />
            </button>
          </div>
          {uncatOpen && (
            uncategorized.length === 0 ? (
              ...
              <div className="flex min-h-0 flex-1 flex-col divide-y divide-border/30 overflow-y-auto no-scrollbar px-3 pb-3">
                {filteredUncategorized.length === 0 ? (
                  <p className="py-4 text-center text-xs text-muted-foreground">No matches</p>
                ) : filteredUncategorized.map(s => (
    ...
    <div className="flex h-full flex-col gap-4">
      <div className="flex shrink-0 items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href="/bor/budget-control/manage"
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Category Assignment</h1>
            <p className="text-sm text-muted-foreground">Define categories per project type and assign subcontractors</p>
          </div>
        </div>
