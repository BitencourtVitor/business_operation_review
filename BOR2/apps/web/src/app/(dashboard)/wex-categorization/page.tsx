...
import {
  ...
  ArrowLeftRight, Ban, Users, ChevronDown, Check,
} from "lucide-react"
import {
...
function buildQbIndex(qbRows: QbRow[], ignoredObras: Set<string> = new Set()): Map<string, string[]> {
  const map = new Map<string, string[]>()
  for (const r of qbRows) {
    const key = `${r.date}|${r.fullNameNorm}`
    const existing = map.get(key) ?? []
    // When an ignored-address set is active, filter individual obra codes
    // from this QB Time entry before adding it to the index.
...
function FileDropZone({ label, accept, fileName, status, warnMessage, onFile, onRemove }: {
  label:         string
  accept:        string
  fileName:      string
  status:        "idle" | "ok" | "warn"
  ...
  return (
    ...
    >
      ...
      {/* X / remove button — shown in ok state so user can clear and re-upload */}
      {status === "ok" && onRemove && (
        <button
          type="button"
...
function ReportCard({ report, onDelete, onExportExcel, onExportPdf, onCompanyChange }: {
  report: WexReport
  onDelete: () => void
  onExportExcel: () => void
  onExportPdf: () => void
  ...
  const [resultsCache,       setResultsCache]       = useState<ResultRow[] | null>(null)
  ...
  async function ensureResults() {
    ...
    try {
      ...
      setResultsCache((full.results ?? []) as ResultRow[])
...
// ─── Ignored Addresses Sheet ─────────────────────────────────────────────────

function IgnoredAddressesSheet({ open, onClose, initialCompany }: {
  open: boolean; onClose: () => void; initialCompany: CompanyFilter
}) {
  const [company, setCompany] = useState<Company>(initialCompany === "all" ? "framing" : initialCompany)
...
function NewReportSheet({ open, onClose, company, onSave }: {
  ...
  // Load norm + ignored addresses from API on open
  useEffect(() => {
    if (!open) {
  ...
  async function handleQbFile(file: File) {
    try {
      ...
      setQbFileName(file.name)  // always show the file name, even on error
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Settings — driver mapping + ignored addresses */}
          <DropdownMenu>
