        ...
        return (
          <div key={account}>
            ...
            <div className="flex items-center justify-between gap-2 px-3 py-0.5">
              ...
              <span className="text-[10px] font-semibold tabular-nums text-muted-foreground">{fmtUSD(groupTotal)}</span>
            </div>
            {/* Lines */}
            {items.map((line, i) => (
              <div key={i} className="flex items-start justify-between gap-2 px-5 py-0.5">
                ...
                <span className="shrink-0 text-[10px] tabular-nums text-muted-foreground/70">{fmtUSD(line.amount)}</span>
              </div>
            ))}
          </div>
        )
    ...
    <div className="mx-3 mb-2 rounded-md border border-border/30 bg-background/40 overflow-hidden">
      ...
      {payments.map((p, i) => (
        <div key={i} className="flex items-center gap-3 px-3 py-1 text-[10px]">
          ...
          <span className="ml-auto shrink-0 font-semibold tabular-nums text-red-400">{fmtUSD(p.total_amount)}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div className="mx-3 mb-2 rounded-md border border-border/30 bg-background/40 overflow-hidden">
      ...
      {payments.map((p, i) => (
        <div key={i} className="flex items-center gap-3 px-3 py-1 text-[10px]">
          ...
          {p.private_note && <span className="flex-1 truncate italic text-muted-foreground/50">{p.private_note}</span>}
          <span className="ml-auto shrink-0 font-semibold tabular-nums text-green-500">{fmtUSD(p.total_amount)}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="rounded-lg border border-border/40 overflow-hidden bg-card/30">
      ...
      >
        ...
        <span
          className="flex w-24 shrink-0 items-center justify-end border-l border-border/30 py-2 pr-3 text-[11px] font-semibold tabular-nums"
          style={{ color }}
        >
          {fmtUSD(amount)}
        </span>
    ...
    <div className="rounded-lg border border-border/50 overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-1 items-center justify-end gap-3">
          <span className="text-xs font-bold tabular-nums">{fmtUSD(estimate.total_amount)}</span>
          <ChevronRight className={cn(
            ...
            <div key={i} className="flex items-start justify-between gap-3 px-5 py-1.5">
              ...
              <div className="flex shrink-0 items-center gap-3 text-[11px] tabular-nums text-muted-foreground">
                {line.quantity > 0 && line.unit_price > 0 && (
                  <span>{line.quantity} × {fmtUSD(line.unit_price)}</span>
                )}
                <span className="font-semibold text-foreground w-20 text-right">{fmtUSD(line.amount)}</span>
              </div>
            </div>
          ))}
        </div>
          ...
          return (
            <EntityCard
              ...
              left={<>
                ...
                {inv.balance > 0 && (
                  <span className="shrink-0 rounded-full bg-yellow-500/10 px-1.5 text-[9px] font-semibold text-yellow-600">
              ...
              <div className="grid grid-cols-2 items-start gap-3">
                ...
                <div className="flex flex-col rounded-xl border border-border bg-muted/10">
                  <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
                    ...
                    <span className="text-xs font-bold tabular-nums text-red-400">{fmtUSD(totalExpenses)}</span>
                  </div>
                ...
                <div className="flex flex-col rounded-xl border border-border bg-muted/10">
                  <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
                    ...
                    <span className="text-xs font-bold tabular-nums text-green-400">{fmtUSD(totalInvoiced)}</span>
                  </div>
                  <div className="flex flex-col gap-2 p-3">
                    <RevenueColumn
