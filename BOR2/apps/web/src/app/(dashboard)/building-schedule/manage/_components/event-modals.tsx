...
import { CalendarRange, Check, FileText, Loader2, Trash2, X } from "lucide-react"
...
export function AddEventModal({ building, onClose }: { building: BuildingListItem; onClose: () => void }) {
      ...
      <div className="bg-background rounded-xl border border-border shadow-xl w-full max-w-md mx-4">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {eventTypes.map(et => (
                    <button key={et.id} type="button" onClick={() => setTypeId(et.id)}
                      ...
                      <EventIcon name={et.icon} className="h-3.5 w-3.5 shrink-0" style={{ color: et.color }} />
                      {et.name}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex-1">
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <CalendarUI mode="single" selected={date ? new Date(date + "T12:00:00") : undefined}
                    onSelect={d => {
                      if (!d) return
        ...
        <div className="flex justify-end gap-2 px-5 pb-5">
          ...
          <Button size="sm" onClick={handleSave} disabled={!canSave}>
            {saving ? <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> : <Check className="h-3.5 w-3.5 mr-1.5" />}Log Event
          </Button>
        </div>
      </div>
    </div>
...
export function EditEventModal({ building, event, onClose }: { building: BuildingListItem; event: ScheduleEvent; onClose: () => void }) {
  const { data: eventTypes = [], isLoading: typesLoading } = useEventTypes()
...
export function RemoveScheduleModal({ building, onClose }: { building: BuildingListItem; onClose: () => void }) {
  const delSched = useDeleteSchedule()
  const [deleting, setDeleting] = useState(false)

  async function handleRemove() {
    setDeleting(true)
    try { await delSched.mutateAsync(building.id); onClose() }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-background rounded-xl border border-border shadow-xl w-full max-w-sm mx-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="font-semibold text-sm text-destructive">Remove Schedule?</h2>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-5">
          <div className="flex items-start gap-3 rounded-lg border border-border bg-muted/20 p-3 mb-4">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium truncate">{building.pdf_filename}</p>
              {building.uploaded_at && <p className="text-xs text-muted-foreground mt-0.5">Uploaded {fmtDateStr(building.uploaded_at)}</p>}
              {building.task_count != null && <p className="text-xs text-muted-foreground">{building.task_count} tasks</p>}
            </div>
          </div>
          <p className="text-xs text-muted-foreground mb-4">This will permanently remove the schedule and all associated row metadata, comments and events.</p>
          <div className="flex justify-end gap-2">
            <Button variant="outline" size="sm" onClick={onClose} disabled={deleting}>Cancel</Button>
            <Button variant="destructive" size="sm" onClick={handleRemove} disabled={deleting}>
              {deleting ? <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5 mr-1.5" />}Remove
            </Button>
          </div>
        </div>
      </div>
...
export function DeleteEventModal({ building, event, onClose }: { building: BuildingListItem; event: ScheduleEvent; onClose: () => void }) {
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                {event.days_delayed > 0 && (
                  <span className="text-[10px] font-medium bg-amber-500/15 text-amber-600 dark:text-amber-400 px-1.5 py-0.5 rounded-full">+{event.days_delayed}d</span>
                )}
              </div>
