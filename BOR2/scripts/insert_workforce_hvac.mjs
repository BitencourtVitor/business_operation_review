    ...
    for (const month of months) {
      ...
      const uploadRes = await client.query(
        `INSERT INTO workforce_uploads (reference_month, company, file_name, record_count, total_hours, uploaded_by, uploaded_at, status)
         ...
         RETURNING id`,
        [month, COMPANY, FILE_NAME, rows.length, totalHours.toFixed(2), 'claude-import']
      );
      ...
      for (let i = 0; i < rows.length; i += chunkSize) {
        ...
        for (const r of chunk) {
          ...
          params.push(uploadId, r.jobcode1 || null, r.jobcode2 || null, r.jobcode3 || null, r.jobcode4 || null, r.employeeName, r.hours, month);
        }
        await client.query(
