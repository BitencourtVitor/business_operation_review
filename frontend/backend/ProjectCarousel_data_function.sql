...
CREATE OR REPLACE FUNCTION get_pcg_project_carousel_data_fixed(
  p_date_from TEXT DEFAULT NULL,
  p_date_to TEXT DEFAULT NULL,
  p_only_accepted BOOLEAN DEFAULT TRUE
)
...
bill_lines_for_estimates AS (
  SELECT DISTINCT
    ...
    bl.description
  FROM filtered_estimates e
  INNER JOIN pcg_bill_lines bl ON (
    (bl.customer_id IS NOT NULL AND bl.customer_id = e.customer_id) OR
    (bl.customer_name IS NOT NULL AND bl.customer_name = e.customer_name)
  )
),
...
bills_for_estimates AS (
  SELECT DISTINCT
    ...
    'bill' AS expense_type
  FROM bill_lines_for_estimates ble
  INNER JOIN pcg_bills b ON b.id = ble.bill_id
),
...
purchases_for_estimates AS (
  SELECT DISTINCT
    ...
    'purchase' AS expense_type
  FROM filtered_estimates e
  INNER JOIN pcg_purchase_lines pl ON (
    (pl.customer_id IS NOT NULL AND pl.customer_id = e.customer_id) OR
    (pl.customer_name IS NOT NULL AND pl.customer_name = e.customer_name)
  )
  INNER JOIN pcg_purchases purch ON purch.id = pl.purchase_id
),
...
vendor_credits_for_estimates AS (
  SELECT DISTINCT
    ...
    'vendor_credit' AS expense_type
  FROM filtered_estimates e
  INNER JOIN pcg_vendor_credit_lines vcl ON (
    (vcl.customer_id IS NOT NULL AND vcl.customer_id = e.customer_id) OR
    (vcl.customer_name IS NOT NULL AND vcl.customer_name = e.customer_name)
  )
  INNER JOIN pcg_vendor_credits vc ON vc.id = vcl.vendor_credit_id
),
...
invoices_for_estimates AS (
  SELECT DISTINCT
    ...
    i.txn_date
  FROM filtered_estimates e
  INNER JOIN pcg_invoices i ON i.customer_id = e.customer_id
),
...
bill_payments_for_estimates AS (
  SELECT DISTINCT
    ...
    bp.private_note
  FROM bills_for_estimates bfe
  INNER JOIN pcg_bill_payment_links bpl ON bpl.txn_id = bfe.external_id
  INNER JOIN pcg_bill_payments bp ON bp.id = bpl.bill_payment_id
),
...
payments_for_estimates AS (
  SELECT DISTINCT
    ...
    pay.private_note
  FROM invoices_for_estimates ife
  INNER JOIN pcg_payment_links pl ON pl.txn_id = ife.external_id AND pl.txn_type = 'Invoice'
  INNER JOIN pcg_payments pay ON pay.id = pl.payment_id
),
