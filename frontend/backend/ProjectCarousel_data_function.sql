-- Função SQL para alimentar o carrossel de projetos (AcceptedEstimatesCarousel)
...
-- 4. Buscar invoices relacionadas aos estimates (mesmo customer_id)
...
-- 5. Buscar bill_payments relacionados às bills (via bill_payment_links)
...
-- 6. Buscar payments relacionados às invoices (via payment_links)
...
-- 7. Agregar dados de bills por estimate
bills_summary AS (
  SELECT
    estimate_id,
    COUNT(DISTINCT bill_id) AS bill_count,
    COALESCE(SUM(total_amount), 0) AS bill_total
  FROM bills_for_estimates
  GROUP BY estimate_id
),

-- 8. Agregar dados de invoices por estimate
invoices_summary AS (
  SELECT
    estimate_id,
...
-- 9. Agregar dados de bill_payments por estimate
...
-- 10. Agregar dados de payments por estimate
payments_summary AS (
  SELECT
    estimate_id,
...
-- 11. Resultado final combinando todos os dados
SELECT
  e.id AS estimate_id,
  ...
  COALESCE(bs.bill_total, 0) AS bill_total,
...
FROM filtered_estimates e
LEFT JOIN bills_summary bs ON bs.estimate_id = e.id
