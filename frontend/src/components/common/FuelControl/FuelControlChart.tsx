...
ChartJS.register({
  ...
  afterDraw: (chart: any) => {
    ...
    chart.data.datasets.forEach((dataset: any, datasetIndex: number) => {
      ...
      const textHeight = 12; // Reduzido de 16 para 12
      const padding = 6; // Reduzido de 8 para 6
      
      // Calcular posição da badge
      let badgeX = x;
      let badgeY = y - textHeight - padding - 20; // Aumentado de 10 para 20
...
export function FuelControlChart({
  ...
  // Dados para o primeiro gráfico: Custo Total vs Galões Totais
  const totalCostVsGallonsData = useMemo(() => {
    ...
    if (shouldGroupByMonth) {
      // Agrupar por MÊS
      const monthlyData = new Map<string, { cost: number; fuel: number }>();
      ...
      return {
        ...
        datasets: [
          ...
          },
          {
            label: 'Total Gallons',
            data: fullMonths.map(monthKey => {
              const monthData = monthlyData.get(monthKey) || { fuel: 0 };
      ...
      // Agrupar por DIA
      const dailyData = new Map<string, { cost: number; fuel: number }>();
     ...
     if (shouldGroupByMonth) {
       ...
       if (!selectedYear && sortedMonths.length > 0) {
         ...
         fullMonths = range;
              ...
              // Sempre incluir TODOS os motoristas que aparecem nos dados (não incluir motoristas sem dados)
        const allDrivers = new Set<string>();
        
        // Adicionar apenas motoristas que realmente têm dados para o período
  ...
  }, [filteredWex, selectedMonth, selectedYear, normalizeName]);

  // Opções para o segundo gráfico
  const driverCostOptions = useMemo(() => {
    let borderDivider = '#e0e0e0';
    try {
      if (typeof document !== 'undefined' && document.documentElement) {
        borderDivider = getComputedStyle(document.documentElement).getPropertyValue('--color-border-divider').trim() || '#e0e0e0';
  ...
  }, [selectedYear, selectedMonth, selectedDrivers, driverCostData]);

  return (
    <>
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'row', minHeight: 0 }}>
        {/* Primeiro gráfico: Custo Total vs Galões Totais - 50% da largura */}
        <div style={{ 
          flex: 1, 
          minWidth: 0,
        ...
        <div style={{ 
          ...
          <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
            {driverCostData && driverCostOptions ? (
              <Line data={driverCostData} options={driverCostOptions} />
            ) : (
              <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ 
                  display: 'flex', 
