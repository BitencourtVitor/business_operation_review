...
const AccountingTableModal: React.FC<AccountingTableModalProps> = ({ show, onClose, data, years = [] }) => {
  // Todas as colunas disponíveis
  const allColumns = useMemo(() => [
    { value: 'date', label: 'Date' },
    { value: 'inv_num', label: 'Invoice' },
    { value: 'bill_num', label: 'Bill' },
    { value: 'customer_full_name', label: 'Customer' },
    { value: 'vendor_display_name', label: 'Vendor' },
  ...
  const [sortBy, setSortBy] = React.useState<typeof allColumns[number]['value']>('open_balance');
  ...
  const availableMonths = useMemo(() => {
    ...
    const yearData = data.filter(row => {
      const rowYear = new Date(row.date || row.date_field || '').getFullYear().toString();
    ...
    yearData.forEach(row => {
      const rowDate = new Date(row.date || row.date_field || '');
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    filteredData.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'open_balance':
          aValue = a.open_balance || 0;
          bValue = b.open_balance || 0;
          break;
        case 'aging_days':
          aValue = a.aging_days || 0;
          bValue = b.aging_days || 0;
          break;
        case 'date':
          aValue = new Date(a.date || a.date_field || '').getTime();
          bValue = new Date(b.date || b.date_field || '').getTime();
          break;
        case 'customer_full_name':
          aValue = (a.customer_full_name || '').toLowerCase();
          bValue = (b.customer_full_name || '').toLowerCase();
          break;
        case 'vendor_display_name':
          aValue = (a.vendor_display_name || '').toLowerCase();
          bValue = (b.vendor_display_name || '').toLowerCase();
          break;
        case 'category':
        ...
        case 'type':
          ...
          break;
        default:
          aValue = (a.customer_full_name || a.vendor_display_name || '').toLowerCase();
          bValue = (b.customer_full_name || b.vendor_display_name || '').toLowerCase();
      }
        ...
        <div style={{ border: '1px solid var(--color-border-divider)', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ borderBottom: '1px solid var(--color-border-divider)', background: 'var(--color-background-primary)', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <span style={{ color: 'var(--color-text-secondary)', margin: 0, fontSize: 24, fontWeight: 400 }}>Accounting</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--color-background-secondary)', borderRadius: 25, padding: '6px 6px 6px 15px', border: '1px solid var(--color-border-divider)', height: 40 }}>
                  ...
                  <button onClick={() => setSortDir(d => d === 'asc' ? 'desc' : 'asc')} style={{ background: 'var(--color-background-primary)', color: 'var(--color-accent-primary)', border: '1px solid var(--color-border-divider)', borderRadius: 15, padding: '4px 10px', fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', height: 26 }}>
                    {(() => {
                      ...
                      const textCols = ['customer_full_name', 'vendor_display_name', 'category', 'type', 'inv_num', 'bill_num', 'transaction_type', 'date'];
                      ...
                      {allColumns.map(column => (
                        visibleColumns.includes(column.value) && (
                          <th key={column.value} style={{ 
                            ...
                            color: 'var(--color-text-primary)', 
                            textAlign: column.value === 'open_balance' || column.value === 'aging_days' ? 'right' : 'left', 
                            position: 'sticky', 
                            top: 0, 
                            background: 'var(--color-background-secondary)', 
                            zIndex: 2 
                        ...
                        {allColumns.map(column => (
                          visibleColumns.includes(column.value) && (
                            <td key={column.value} style={{ 
                              padding: 12, 
                              border: '1px solid var(--color-border-divider)', 
                              color: column.value === 'open_balance' ? 'var(--color-accent-primary)' : 'var(--color-text-secondary)', 
                              textAlign: column.value === 'open_balance' || column.value === 'aging_days' ? 'right' : 'left',
                              fontWeight: column.value === 'open_balance' ? 500 : 400
                            }}>
                              {column.value === 'open_balance' ? 
                                (row.open_balance || 0).toLocaleString('en-US', { 
                                  style: 'currency', 
                                  currency: 'USD' 
                                }) :
                                column.value === 'date' || column.value === 'due_date' ? 
                                  (row[column.value as keyof AccountingRow] ? dayjs(row[column.value as keyof AccountingRow] as string).format('DD/MM/YYYY') : '-') :
                                column.value === 'aging_days' ? 
                                  (row.aging_days || 0).toString() :
                                row[column.value as keyof AccountingRow] as string || '-'
                              }
                            </td>
                          )
                        ))}
...
export default AccountingTableModal; 