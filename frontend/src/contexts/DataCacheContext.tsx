  ...
  const fetchProjectMonitoringHvacData = async () => {
    ...
    try {
      ...
      const projectMonitoringData = (dbData || []).map((row) => ({
        ...
        s1_date: row.s1_date ? new Date(row.s1_date).toISOString().split('T')[0] : '',
        s2_machines: String(row.s2_machines ?? ''),
        s2_date: row.s2_date ? new Date(row.s2_date).toISOString().split('T')[0] : '',
        s3_condenser: String(row.s3_condenser ?? ''),
        s3_date: row.s3_date ? new Date(row.s3_date).toISOString().split('T')[0] : '',
        s4_finish: String(row.s4_finish ?? ''),
        s4_date: row.s4_date ? new Date(row.s4_date).toISOString().split('T')[0] : '',
